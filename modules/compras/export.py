"""
Exporta el listado de Compras desde Advertys: login + navegar
(Administracion > Compras, TreeView por texto/title) + filtro "Todas" +
descarga XLSX. Version productiva de explore.py: mismos selectores ya
verificados contra Advertys real, sin las capturas de debug intermedias
(solo se guarda una si algo falla, para diagnostico).

Uso:
    python -m modules.compras.export
"""
import sys
    ...
    with sync_playwright() as p:
        ...
        try:
            try:
                ...
                page.screenshot(path=str(SCREENSHOT_DIR / "compras_export_error_login.png"))
                raise

            _click_por_texto_o_title(page, "Administracion")
            page.wait_for_timeout(800)

            if not _click_por_texto_o_title(page, "Compras"):
                page.screenshot(path=str(SCREENSHOT_DIR / "compras_export_error_menu.png"), full_page=True)
                raise ExportError("No se encontro el nodo 'Compras' en el menu.")
            ...
            try:
                ...
                page.screenshot(path=str(SCREENSHOT_DIR / "compras_export_error_download.png"), full_page=True)
                raise ExportError(f"Error exportando Compras: {e}") from e
