...
de .env, navega DIRECTO por URL a "OCs Pendientes de Generar" (shortcut del
menu "Cuentas y Produccion") y exporta a Excel.

Mismo motivo que modules/estimados_costos/explore.py para navegar por URL
directa: evita depender de que el shortcut del menu lateral abra el
listado (algunos shortcuts de esta seccion disparan formularios de alta en
vez del listado).
...
Si Advertys le agrega uno en el futuro y las filas parecen menos de las
esperadas, revisar si hay que ponerlo en la opcion mas amplia (mismo gotcha
que Facturas/Estimado Costos).

Uso:
    python -m modules.oc_pendientes_generar.explore
"""
import os
import sys
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if "Login.aspx" in page.url:
            ...
            page.screenshot(path=str(SCREENSHOT_DIR / "oc_pendientes_error_login.png"))
            browser.close()
            sys.exit(1)

        print("Login OK. Navegando directo a 'OCs Pendientes de Generar'...")
        base_url = URL.split("Login.aspx")[0]
        direct_url = f"{base_url}Default.aspx#ViewID=EstimadoDetalle_ListView_OC_Pendientes&ObjectClassName=DPS_SAS_SR.Module.EstimadoDetalle"
        page.goto(direct_url, wait_until="networkidle", timeout=30000)
        esperar_postback(page)
        page.wait_for_timeout(800)
        page.screenshot(path=str(SCREENSHOT_DIR / "oc_pendientes_01_listado.png"), full_page=True)
        ...
        try:
            ...
            destino = OUT_DIR / "oc_pendientes_generar_export.xlsx"
            download.save_as(str(destino))
            print(f"OK: descarga guardada en {destino}")
        except Exception as e:
            print(f"ERROR al descargar: {e}")
            page.screenshot(path=str(SCREENSHOT_DIR / "oc_pendientes_error_export.png"), full_page=True)
