"""
Exporta el listado de Orden Publicidad (vista Navegacion) desde Advertys:
login + navegar DIRECTO por URL + filtro "Todos" + descarga XLSX. Version
productiva de explore.py: mismos selectores ya verificados contra Advertys
real, sin las capturas de debug intermedias (solo se guarda una si algo
falla, para diagnostico).

Uso:
    python -m modules.ordenes_publicidad.export
"""
import sys
    ...
    with sync_playwright() as p:
        ...
        try:
            try:
                ...
                page.screenshot(path=str(SCREENSHOT_DIR / "ordenes_publicidad_export_error_login.png"))
                raise
            ...
            try:
                ...
                page.screenshot(path=str(SCREENSHOT_DIR / "ordenes_publicidad_export_error_download.png"), full_page=True)
                raise ExportError(f"Error exportando Orden Publicidad: {e}") from e
