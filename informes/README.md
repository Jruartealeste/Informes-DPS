...
Excel exportado de Advertys
   ...
   Base local (SQLite, una tabla por módulo)
        │
        ├──► modules/<modulo>/generate_html_report.py  →  salida/informe_*.html
...
**Entrypoint real: `salida/dashboard.html`.** Agrupa todos los
`salida/informe_*.html` bajo un único sidebar de navegación (estilo
...
`salida/` porque el sidebar los carga en un `<iframe>` por nombre de
...
├── generate_dashboard.py # genera salida/dashboard.html (sidebar + iframe sobre los informe_*.html)
...
│   │   ├── generate_html_report.py   # genera salida/informe_dinamico.html (el informe real)
...
└── salida/             # todos los .html que arma el pipeline, en una sola carpeta
...
|---|---|---|---|
| Órdenes de Trabajo | `modules/ordenes_trabajo/` | `ordenes_trabajo` | `salida/informe_dinamico.html` |
| Compras (Administración > Compras) | `modules/compras/` | `compras` | `salida/informe_compras.html` |
| Facturas (Consultas > Facturación > Facturas) | `modules/facturas/` | `facturas` | `salida/informe_facturas.html` |
...
- **Origen del pedido (Javier, 2026-08-04):** "a partir de lo que cobramos
  vemos qué proveedores vamos a pagar" — cadena Cobrado (Recibo Cliente) →
  Facturado (factura de venta que ese recibo cancela) → Detalle de lo
  Facturado (N° de OC/OP de esa factura → proveedor, monto, saldo, estado).
  Confirmado con Javier que hacía falta trazabilidad hasta el recibo
  puntual (no alcanzaba con las columnas Facturado/Comprado/Cobrado/Pagado
  que ya trae `ordenes_compra_produccion` por OC, agregadas pero sin usar
  en ningún informe hasta ahora).
- **Relevado en vivo 2026-08-04:** el listado de Recibo Cliente
  (Administración > Recibo Cliente, `ViewID=IC_ReciboCliente_ListView`)
  exporta en bloque sin problema (405 recibos, histórico completo, sin
  volumen para justificar ventana rodante en la ingesta misma) — clave
  única de una sola columna, `Nº Recibo` (a diferencia de
  Compras/Facturas/O.C. Producción, que necesitan clave compuesta). Usa
  `º` (U+00BA) como Compras/O.C. Producción, no `°` (U+00B0) como
  Facturas/IIBB — mismo gotcha de siempre, dos vistas de "Administración"
  comparten convención.
- **Pero la pestaña "Referencias Canceladas" (el detalle de qué factura(s)
  cancela cada recibo) NO exporta en bloque** — confirmado en vivo, misma
  situación que "Items Facturas" en IIBB: hay que abrir cada recibo
  individual. Por eso `modules/recibos/crawl_referencias_canceladas.py`
  crawlea recibo por recibo con Playwright (solo lectura), igual patrón que
  `modules/iibb/crawl_oc_por_factura.py`. Esa sub-grilla trae `TR` +
  `N° Referencia` (con `°`, U+00B0 — esta sí, porque referencia a una
  Factura) pero **no** trae `TA` ni `N° Asiento`: no alcanza sola para
  reconstruir `facturas.clave_factura` completa. Se desambigua en
  `generate_html_report.py` comparando el monto aplicado por el recibo
  contra `facturas.total_ml` de cada candidata; si dos candidatas quedan
  igual de cerca, la fila se marca "Revisar (cruce ambiguo)" en vez de
  adivinar (decisión confirmada con Javier).
- **Ventana bajada de 6 a 2 meses el mismo día (Javier, 2026-08-04):**
  arrancó en 6 meses (mismo criterio que IIBB) pero con 71 recibos el
  crawl de Referencias Canceladas no terminaba en varios minutos — a
  diferencia de IIBB (una sola página por factura), acá cada recibo implica
  volver al listado, reabrir el filtro y re-buscar, así que el costo por
  recibo es más alto. Con 2 meses (~23 recibos) el crawl completo tarda
  unos minutos. "No son muchas cobranzas" (Javier) — alcanza para el uso
  operativo real ("qué pago ahora con lo que acabo de cobrar").
- **Bug real de navegación (detectado y corregido 2026-08-04):** en el caso
  raro de un mismo N° Referencia repetido entre dos facturas con TA
  distinto (ver nota de IIBB arriba — confirmado que pasa también con
  000500000538 y 000500000542 en esta ventana, no solo 000500000015/016),
  `modules/cobranza_proveedores/crawl_oc_por_factura.py` necesita abrir
  las dos filas candidatas por separado. La primera versión buscaba el
  texto en TODA la página (`page.get_by_text(...).nth(idx)`) y clickeaba
  por índice — pero el segundo click terminaba clickeando la MISMA fila que
  el primero (confirmado comparando contra `facturas.tipo_asiento` local:
  para 000500000015 hay una factura ALUAR/FM y una INCAA/FP realmente
  distintas, pero el crawl traía la de ALUAR duplicada). Se corrigió
  acotando el click a la fila real de la grilla (`tabla_visible()` +
  `tr[id*='DXDataRow'].nth(idx)`, clickeando la celda con el texto buscado
  DENTRO de esa fila puntual) en vez de un match de texto global — clickear
  a ciegas la primera celda de la fila tampoco sirve (no dispara el
  handler de apertura, quedó probado y descartado en el camino).
- **Gap real: "Órdenes de Compra/Publicidad" son DOS entidades de Advertys
  con rangos de numeración separados** (confirmado 2026-08-04): la columna
  "Orden Compra" de una factura Producción (TA=FP) trae números de
  `ordenes_compra_produccion` (rango 2-205), pero la misma columna en una
  factura Medios (TA=FM) trae números de **Orden de Publicidad** (rango
  7000+, ej. "7023 - El Bajo Producciones SRL -$57463770,15 -
  Autorizada"). La primera versión de `_oc_por_factura()` no tenía módulo
  propio para esa segunda entidad y parseaba ese texto crudo por regex
  (`orden_compra_raw`) como único dato disponible — **cerrado el mismo
  día** sumando `modules/ordenes_publicidad/` (ver tabla de módulos
  arriba), que releva la vista real de Advertys (Medios > Ordenes
  Publicidad > Navegación). El parseo de texto crudo sigue vivo solo como
  último recurso si una OP referenciada no aparece en esa tabla. La
  columna "Origen OC" del informe marca `Producción` / `Publicidad` /
  `Publicidad (año ambiguo)` / `Publicidad (estimado)` según la fuente y
  certeza de cada fila (ver próximo punto).
- **Gotcha real de `ordenes_publicidad`: el número de "Orden" no es clave
  única por sí solo** — es un correlativo que se reinicia cada año (Orden
  1001 existe tanto en 2025 como en 2026, con proveedores distintos), pero
  el texto crudo que ya cruza `cobranza_proveedores` (`orden_compra_raw`,
  y por lo tanto `items_oc.numero_oc`) sólo trae el número pelado, sin
  año. En una muestra real, de 164 `numero_oc` referenciados desde
  facturas, 116 (71%) colisionaban con más de una fila en
  `ordenes_publicidad` si se buscaba sin año. `_resolver_op()` en
  `modules/cobranza_proveedores/generate_html_report.py` desambigua
  cruzando también por Proveedor (mismo texto ya parseado): baja la
  colisión a 12/164 (7%), que se resuelven con la fila de año más reciente
  como mejor esfuerzo y quedan marcadas `Publicidad (año ambiguo)` en la
  columna "Origen OC" en vez de mostrarse como si fueran un dato cierto.
- **"Saldo a Pagar" = importe sin IVA de la OC/OP, siempre (corregido
  2026-08-04, pedido explícito de Javier: "sin importar si está utilizada
  o autorizada el estado que tenga siempre el valor de las órdenes de
  compra, es decir el monto sin IVA"):** la primera versión de
  `_oc_por_factura()` neteaba mal esta columna de dos formas distintas —
  para OC de Producción tomaba el `saldo` que ya trae Advertys (no
  garantizado sin IVA, y afectado por cuánto ya se pagó/compró), y para OP
  parseadas del texto crudo, ese mismo texto ya traía el SALDO actual (no
  el importe original) y encima se pisaba a 0 en estados terminales.
  Confirmado en datos reales: el 100% de las OP en estado "Utilizada"
  mostraban literalmente "-$ 0,00 -" en el texto crudo de Advertys. Ahora
  `oc_saldo` es siempre igual a `oc_importe_sin_iva`: dato real de
  `ordenes_compra_produccion.importe_sin_iva` para Producción, o de
  `ordenes_publicidad.importe_sin_iva` (columna "Total Orden" de Advertys,
  confirmada independiente del estado) para Publicidad — sin condicional
  por estado en ningún caso. El total de "saldo OC pendiente" del informe
  subió de ~$329M a ~$1.271M con este fix, casi todo por las OP en
  "Utilizada" que antes se contaban en $0.
- **Bug real de doble conteo (detectado y corregido 2026-08-04):** una
  factura con varias OC vinculadas genera varias filas en la tabla de
  detalle (una por OC, para poder listar cada proveedor/saldo aparte) — un
  stat tile/chart que sumara `monto_aplicado` tal cual quedaba multiplicado
  por esa cantidad de OC (el primer intento mostró "Total cobrado
  $7.956.904.076" contra los ~$1.990.545.248 reales, ver
  `_dup_referencia`/`monto_cobrado_unico` en `generate_html_report.py`:
  el monto solo se cuenta en la primera fila de cada combinación
  recibo+factura, 0 en las repetidas).
- **Sanity check real:** el saldo pendiente de la única OC de Producción
  con saldo > 0 en esta ventana (OC #198, SADAIC, $677.000, estado
  "Autorizada") coincide exactamente con el caso ya documentado en la
  sección de Pendientes de este mismo README (OT 278) — buena señal de que
  el cruce completo (Recibo → Factura → OC) está bien armado.
- **Filtros categóricos (`hr.category_filters_html`, `html_report.py`) —
  primera vez que se usan en el proyecto (Javier, 2026-08-04):** dropdowns
  "Todos" + valores únicos del dato (acá: Cliente, N° Recibo, N° Factura,
  Proveedor, Estado OC, Origen OC), poblados en `DASHBOARD_JS` desde
  `records` sin pasar opciones desde Python. Actúan sobre
  statTiles/charts/tabla por igual (`applyCategoryFilters`, encadenado
  después del filtro de período en `renderAll`), a diferencia del buscador
  en vivo de cada tabla (`renderTable`) que solo filtra esa tabla puntual.
  Reutilizable para cualquier módulo futuro que necesite filtrar por una
  columna categórica, no solo por período.
- **Tabla agrupada con detalle desplegable (`t.mode = "grouped"`,
  `html_report.py` — primera vez que se usa en el proyecto, Javier,
  2026-08-04):** pedido explícito de comprimir la tabla a una fila por
  recibo (con Facturas/Total Cobrado/Proveedores/Saldo a Pagar agregados) y
  desplegar el detalle de OC/proveedor por factura al clickear. Distinto
  del drill-down estático de Pendientes (`<details>`-like con
  `_ROW_TOGGLE_JS`, HTML fijo generado una vez): acá el agrupado tiene que
  recalcularse en cada cambio de filtro (período o categórico), así que se
  armó `groupRows`/`renderGroupedTable`/`paintGroupedTable` en
  `DASHBOARD_JS` en vez de generarlo server-side. `groupRows` soporta los
  ops `first`/`count`/`sum`/`nunique` (mismo vocabulario que `aggregate()`,
  pero conservando las filas originales del grupo en `_rows` para el
  detalle) más `sum_unique` (suma un campo una sola vez por valor distinto
  de otro campo — necesario para "Saldo a Pagar" del grupo: una misma OC
  puede repetirse en más de una línea de detalle del mismo recibo, ej. dos
  items de la misma factura con el mismo N° de OC, y sumar `oc_saldo` tal
  cual la contaría dos veces). Reutilizable para cualquier módulo futuro
  con esta misma forma (cabecera + detalle N a 1).
...
python -m modules.ordenes_trabajo.generate_html_report         # genera salida/informe_dinamico.html
```
...
python -m modules.recibos.ingest "ruta/a/tu/export.xlsx"       # cabecera Recibo Cliente
python -m modules.recibos.crawl_referencias_canceladas          # sin Excel, navega Advertys en vivo (~pocos minutos con la ventana de 2 meses)
...
clave única. Después abrís `salida/dashboard.html` (o el
`salida/informe_*.html` suelto) con doble click y usás el botón
"Imprimir / Guardar PDF" (o Ctrl+P) si necesitás mandárselo a un cliente.

Para que `salida/informe_pendientes.html` refleje datos frescos hace
...
Para `salida/informe_cobranza_proveedores.html`, el orden importa: primero
`modules.recibos.ingest` (cabecera) y recién ahí
`modules.recibos.crawl_referencias_canceladas` (lee la ventana de 2 meses
de la tabla `recibos` ya cargada); después
`modules.cobranza_proveedores.crawl_oc_por_factura` (lee las facturas
referenciadas por `referencias_canceladas`); recién con eso corrido tiene
sentido `modules.cobranza_proveedores.generate_html_report` — también
necesita `facturas` y `ordenes_compra_produccion` actualizados (sus
propios `ingest.py`, arriba) para el detalle completo.

`salida/dashboard.html` en sí casi nunca hace falta regenerarlo — solo
...
descarga vía Playwright) para 8 de los 9 módulos:
...
`estimados_costos`, `ordenes_compra`, `ordenes_publicidad`,
`oc_pendientes_generar`, `estimados_pendientes_facturar` y
`ordenes_trabajo` (cada uno una versión
...
| `generate_dashboard.py` | Genera `salida/dashboard.html`: sidebar con un ítem por módulo dado de alta, cada uno carga su `informe_*.html` (misma carpeta) en un iframe |
