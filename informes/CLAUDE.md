...
- Compartidas (raíz): `db.py` (conexión SQLite), `common.py`
  ...
  (genera `salida/dashboard.html`, el entrypoint que agrupa todos los
  `salida/informe_*.html`), `api.py` (API local de solo lectura).
...
Esta carpeta (`informes/`) es la raíz del pipeline en sí — es hermana de
`ot/` (la app de tareas/OT, otro proyecto, ver más abajo) bajo la carpeta
contenedora del repo. Adentro:
...
salida/                 # dashboard.html + informe_*.html — entregable real, se sobrescribe en cada corrida
.env                    # credenciales de Advertys (NUNCA en otro lado, gitignored)
```

**Deliverable real:** los `informe_<modulo>.html` en `salida/` — son
autocontenidos (sin CDN), se abren con doble click y se imprimen a PDF
desde el navegador. `salida/dashboard.html` los agrupa con un `<iframe>`
...
**Esta sección aplica al pipeline de informes** (`modules/`, `tools/`,
`explore.py` de cada módulo, y los scripts compartidos en la raíz de este
CLAUDE.md) — no a `ot/`, que es un proyecto hermano con su propia app y su
propio `ot/CLAUDE.md`, donde escribir en Advertys (crear/modificar/eliminar
OTs y estimados) es parte explícita del alcance. Si estás trabajando
dentro de `ot/`, regís por `ot/CLAUDE.md`, no por esta sección.
...
- **Siempre parado en esta carpeta (`informes/`), no en la carpeta
  contenedora del repo:** los módulos se corren con
  `python -m modules.<modulo>.<script>`, nunca `python archivo.py` directo.
  Si la sesión arrancó en la raíz del repo (donde también vive `ot/`), el
  primer paso es `cd informes` antes de cualquier comando de este pipeline.
